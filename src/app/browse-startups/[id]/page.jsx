...
export default function StartupDetails() {
  ...
  const showToast = (msg) => {
    ...
    setTimeout(() => {
      setToastMessage("");
    }, 3050); // ৩ সেকেন্ড পর হাইড হবে
  };

  // 🚀 অ্যাপ্লিকেশন সাবমিট করার লজিক
  const handleSubmitApplication = async (e) => {
    ...
    try {
      ...
      if (data.success) {
        ...
        // 🔔 স্ক্রিনশটের মতো পারফেক্ট কাস্টম সাকসেস টোস্ট ট্রিগার
  ...
  return (
    <div className="mx-auto max-w-4xl px-6 py-12 relative">
      ...
      <button 
        onClick={() => router.push("/browse-startups")}
        className="flex items-center gap-2 text-sm font-bold text-slate-600 hover:text-indigo-600 transition mb-8 cursor-pointer"
      >
        <FiArrowLeft size={16} /> Back to Startups
      </button>
      ...
      <div className="bg-white rounded-3xl border border-slate-200 p-8 shadow-sm space-y-8">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-6 pb-6 border-b border-slate-100">
          <div className="flex items-center gap-4">
            <div className="flex h-20 w-20 shrink-0 items-center justify-center rounded-2xl bg-slate-50 text-indigo-600 font-bold text-3xl overflow-hidden border border-slate-200">
              {startup.logo ? (
              ...
              <form onSubmit={handleSubmitApplication} className="space-y-4">
                ...
                <div className="flex items-center justify-end gap-3 pt-2">
                  ...
                  <button 
                    type="submit" 
                    disabled={isSubmitting}
                    className="flex items-center justify-center gap-2 rounded-xl bg-indigo-600 px-5 py-2.5 text-xs font-bold text-white hover:bg-indigo-700 shadow-md hover:shadow-lg hover:shadow-indigo-100 transition cursor-pointer min-w-[110px] disabled:bg-indigo-500"
                  >
                    {isSubmitting ? (
                      <>
      ...
      {/* 🔔 স্ক্রিনশটের মতো পারফেক্ট কাস্টম ডার্ক টোস্ট কন্টেইনার */}
      <AnimatePresence>
        {toastMessage && (
          <motion.div 
